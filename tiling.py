...
def get_tile(index, image, mask, double):
    ...
    tile_width_index, tile_height_index = get_tile_width_height_indexes(tile_index, double=double)
    image = image[
...
def insert_tile_into_picture(tile_height_index, tile_width_index, insert_to_image, tile):
    insert_to_image[
    :,
    tile_height_index: tile_height_index + TILE_SIZE,
    tile_width_index: tile_width_index + TILE_SIZE,
...
def get_masks_from_tiles(predicted_tiles, ground_truth_tiles):
    ...
    for tile_index, predict_and_ground in enumerate(
        ...
        tile_width_index, tile_height_index = get_tile_width_height_indexes(tile_index, double=False)
        whole_predicted = insert_tile_into_mask(tile_height_index, tile_width_index, whole_predicted, predicted_tile)
        whole_ground_truth = insert_tile_into_mask(tile_height_index, tile_width_index, whole_ground_truth, ground_truth_tile)
    return whole_predicted, whole_ground_truth

def get_picture_from_tile(image_tiles):
    whole_image = torch.zeros(
        (4, IMG_SIZE, IMG_SIZE), dtype=torch.float32
    )
    for tile_index, tile in enumerate(image_tiles):
        tile_width_index, tile_height_index = get_tile_width_height_indexes(tile_index, double=False)
        whole_image= insert_tile_into_picture(tile_height_index, tile_width_index, whole_image, tile)
    return whole_image
