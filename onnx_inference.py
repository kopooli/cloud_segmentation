...
if len(sys.argv) not in [1, 3]:
    raise Exception("Invalid number of arguments passed, either pass path/to/images path/to/labels or do not pass anything.")
...
if os.listdir(images_dir) != os.listdir(label_masks_dir):
    raise Exception("Labels does not match input images. There must be same number of them and they must have same names.")
batch_size = TILES_PER_IMAGE
save_images = True
ort_session = onnxruntime.InferenceSession("cloud_segmenter.onnx", providers=["CPUExecutionProvider"])
...
inference_dataset = CloudDataset(images_dir, label_masks_dir, image_names, "test", transform)
n_cpu = os.cpu_count()
